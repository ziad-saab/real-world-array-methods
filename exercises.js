// Use the appropriate array method to figure out, using the hourly data, 
// whether there is going to be any rain in the next 
// 24 hours. The method you use should return true or false.

// precipProbability
// hourly : data : [{"precipProbability": 0},{},{},etc]
...
var hourWeatherData = ioWeather.hourly.data;
// console.log(hourWeatherData);
function willItRain(arr) {
    return arr.some(function(hour) {
        return hour.precipProbability;
     
        //return (hour.precipProbability === 0 ? false : true)
    });
}

//console.log(willItRain(hourWeatherData));



// Using a chain of two array methods, figure out if it's going to rain in the next 8 hours. To do this, 
//you'll have to start with the hourly data and: Eliminate any item where the time is further than 8 hours
//from now. Do the same thing as part 1.

function rainWithin8hrs(arr) {
    //reduce to find if it will rain in the next 8 hours
    // reduce down to array of first 8 data elements
    return arr.reduce(function(newArr, next){
        newArr.push(next.precipProbability);
        var allHours = newArr;
        var eightHours = allHours.filter(function(el,i) {
            if (i<8) {
                return el;
            }
        });
...
//console.log(rainWithin8hrs(hourWeatherData));
...
var hourlyTemp = ioWeather.hourly.data;
...
//console.log(getTemp(hourlyTemp));

// Using a chain of two array methods, start with the hourly data and return an array of Date objects saying 
// when it will rain. To do this, you'll have to: Eliminate any item where it is not raining.
// Extract the time from the remaining items and create a Date object

function isRaining(arr) {
    return arr.filter(function(obj) {
        if (obj.precipIntensity > 0) {
     ...
     map(function(obj) {
        console.log("obj: "+obj);
        var unixTime = [];
        unixTime.push(obj.time);
        return unixTime.forEach(function(time) {
...
console.log(isRaining(hourlyTemp));
