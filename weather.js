...
//check if rain will be there in the next 24 hours 

function isItGoingToRainToday(input){
    //hourly data array
    return Boolean(input.daily.data[0].precipProbability);
}

//console.log(isItGoingToRainToday(input));

function isItGoingToRainSoon(input, hours){
    var rain = false;
    input.hourly.data.filter(function(ele){
        return (ele.time - input.currently.time) < hours * 3600;
    }).forEach(function(ele){
        if (ele.precipProbability) {rain = true}
    });
    return rain;
}

//console.log(isItGoingToRainSoon(input, 24));

function giveMeTempHourly(input){
    var hourlyTemp = [];
    input.hourly.data.forEach(function(ele){
        hourlyTemp.push(ele.temperature)
    })
        return hourlyTemp;
}

//console.log(giveMeTempHourly(input));

function whenIsItGoingToRain (input){
    var rainDates = [];
    input.hourly.data.filter(function(ele){
        return Boolean(ele.precipProbability);
    }).forEach(function(ele){
        rainDates.push({Date: ele.time})
    })
    return rainDates;
}

function whenIsItGoingToRainReduce(input){
    return input.hourly.data.reduce(function(prev, ele){
        if (ele.precipProbability){return prev.concat([{Date: ele.time}])}
        else {return prev}
    },[])
}

//console.log(whenIsItGoingToRain(input));
//console.log(whenIsItGoingToRainReduce(input))
function sunnyAllWeek(input){
    var sunny = true
    input.daily.data.forEach(function(ele){
        if (ele.summary.search("sun") === -1){
            sunny = false
        }
    })
    return sunny;
...
//console.log(sunnyAllWeek(input));

function howHotToday(input){
    return input.hourly.data.filter(function(ele){
        return (ele.time - input.currently.time) < 24 * 3600;
    }).reduce(function(prev, ele){
        if (prev >= ele.temperature){return prev}
        else {return ele.temperature}
    }, -Infinity);
}

//console.log(howHotToday(input));

function weeklyStats(input){
    return input.daily.data.reduce(function(prev, ele){
        if (prev[ele.icon]){prev[ele.icon]++}
        else {prev[ele.icon] = 1}
        return prev;
    }, {})
}

//console.log(weeklyStats(input));