//starting weather methods

//weatherData(object) ---> currently(object), daily(object) --> data(array)
// [then]> flags(object) [then]> hourly(object) --> data(array of objects(hours))
...
function rainCheck(obj) {
   var sumString = obj.hourly.summary.split(" ");
   return sumString.some(function(word){
       return word.toLowerCase() === "rain";
...
// As an additional hint, the first method is filter and the last one is forEach. 
...
function fourChain(arr) {
   var filteredArray = [];
   
             arr.filter(function(obj) {
                    obj.email.match('.biz') ? filteredArray.push(obj.address.city) : false;
                })
...
console.log(unique(fourChain(newDataArray)));
